  ...
  for (y = 0; y != height; ++y) {     ///0~255
    for (x = 0; x != width; ++x) {    ///0~255
          ...
          if (x + u >= 0 && x + u < width && y + v >= 0 && y + v < height) {
            ...
            if(x == 0 || x+u ==  x+2 ){
              ...
              initiator.write_to_socket(SOBEL_MM_BASE + SOBEL_FILTER_R_ADDR, mask,
                                        data.uc, 4);///跟Initiator.h的read_from_socket對接
              R_counter++; /// sobel_filter read 次數
      ...
      */
      ...
      while(!done){  /// 一直在這裡重複read 直到 done(全部的)
        initiator.read_from_socket(SOBEL_MM_BASE + SOBEL_FILTER_CHECK_ADDR, mask,
                                   data.uc, 4);
        output_num = data.sint; /// signed int   o_result.read()
      ...
      initiator.read_from_socket(SOBEL_MM_BASE + SOBEL_FILTER_RESULT_ADDR, mask,
                                 data.uc, 4);
      total = data.sint;
      W_counter++;/// sobel_filter write -> transaction -> testbench 次數
      ///bytes_per_pixel = 3
